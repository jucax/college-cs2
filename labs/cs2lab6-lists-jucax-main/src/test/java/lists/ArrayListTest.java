...
class ArrayListTest {
	// Note that these are from list.ArrayList
	// and are not the standard Java ArrayList
	ArrayList<Integer> intList;
	ArrayList<Integer> smallIntList;

	@BeforeEach
	void setUp() throws Exception {
		intList = new ArrayList<>(6);
		intList.add(77);
		intList.add(-324);
		intList.add(9934);
		intList.add(0);
	...
	void testArrayListInt() {
		...
		assertThrows(IllegalStateException.class, () -> {
			ArrayList<Integer> temp2 = new ArrayList<>(-10);
		});

		assertThrows(IllegalStateException.class, () -> {
			ArrayList<Integer> temp2 = new ArrayList<>(10001);
		});

		ArrayList<Integer> temp2 = null;
		try {
			temp2 = new ArrayList<>(10001);
		} catch(IllegalStateException e) {
			// Prevents an error from crashing program
		}
	}
	...
	void testIterator() {
		...
		assertEquals(Integer.valueOf(77), itr.next());
		assertTrue(itr.hasNext());
		assertEquals(Integer.valueOf(-324), itr.next());
		assertTrue(itr.hasNext());
		assertEquals(Integer.valueOf(9934), itr.next());
		assertTrue(itr.hasNext());
		assertEquals(Integer.valueOf(0), itr.next());
		assertTrue(itr.hasNext());
		assertEquals(Integer.valueOf(1234), itr.next());
		assertTrue(itr.hasNext());
		...
		for(Integer x : intList) {
			assertEquals(intList.getEntry(index++), x);
		}
